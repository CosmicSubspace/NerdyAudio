//Originally "SoundFile" class in the ringdroid source.
...
import com.cosmicsubspace.nerdyaudio.helper.ByteConversions;
...
public class SoundFile {
    ...
    private int mAvgBitRate;  // Average bit rate in k  bps.
    ...
    public void ReadFileMetaData(File inputFile)
            throws java.io.FileNotFoundException,
            java.io.IOException, InvalidInputException {
    ...
    public void ReadFileWithCallback(File inputFile, BufferFeedListener bfl)
        ...
        while (!stop) {
            ...
            if (seekToRequest>=0){
                extractor.seekTo(seekToRequest,MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                seekToRequest=-1;
            }

            //Get the input buffer index to be filled. timeout=100us.
            int inputBufferIndex = codec.dequeueInputBuffer(100);

            if (!done_reading && inputBufferIndex >= 0) {

                sample_size = extractor.readSampleData(inputBuffers[inputBufferIndex], 0); //Read some encoded samples, put it in inputBuffers[index]
                ...
                if (firstSampleData
                    ...
                    done_reading = true;
                } else { //Normally we would end up here.
                    presentation_time = extractor.getSampleTime(); //the time this audio should be played.
                    codec.queueInputBuffer(inputBufferIndex, 0, sample_size, presentation_time, 0); //Put the acquired encoded data into the MediaCodec.
                    extractor.advance(); //and we go to the next sample or frame or something
                    tot_size_read += sample_size;

                    if (mProgressListener != null) { //We don't actually need this (AFN)
            ...
            int outputBufferIndex = codec.dequeueOutputBuffer(info, 100); //get the prepared output buffer index.
            if (outputBufferIndex >= 0 && info.size > 0) { //if it's valid data...
                if (decodedSamplesSize < info.size) { //if the array isn't big enough (or is not initialized)
                    decodedSamplesSize = info.size;
                    decodedSamples = new byte[decodedSamplesSize];
                }
                outputBuffers[outputBufferIndex].get(decodedSamples, 0, info.size);//get some decoded data.
                ...
                //Log.d("CS_AFN","Trying to write");
                //mAudioTrack.write(decodedSamples, 0, decodedSamples.length);
                */

                bfl.feed(ByteConversions.bytesToShorts(decodedSamples)); //callback to AFN player.

                codec.releaseOutputBuffer(outputBufferIndex, false); //and release it.
...
/*
    ...
    public void dumpToPCMFile(File f) throws IOException {
        int startOffset = 0;
